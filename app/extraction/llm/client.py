...
OLLAMA_FALLBACK_MODELS = ["qwen2.5", "granite3.3"]
...
def chat_completion(
    ...
    *,
    ...
    # Se for LLM local (Ollama), incluir fallbacks para 404 "model not found"
        ...
        for attempt in range(max(1, retries + 1)):
            try:
                ...
                if is_model_not_found and try_model != models_to_try[-1]:
                    ...
                    break  # sair do loop de retries e tentar próximo modelo
                if attempt < retries and not is_model_not_found:
                    sleep_time = 2 ** attempt
                    time.sleep(sleep_time)
        else:
            # inner loop esgotou retries sem break; desistir deste modelo
            continue
