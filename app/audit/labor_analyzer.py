...
from .audit_rules import FRASE_ENCARGO_NAO_AUDITAVEL
...
def analyze_labor_charges(df: pd.DataFrame, document_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    valor_base_folha_documento = None
    base_folha_mes_anterior = None
    if document_context and isinstance(document_context, dict):
        valor_base_folha_documento = document_context.get("valor_base_folha_documento")
        base_folha_mes_anterior = document_context.get("base_folha_mes_anterior")
    if valor_base_folha_documento is not None and float(valor_base_folha_documento) > 0:
        result["base_calculo"]["valor_base_folha"] = round(float(valor_base_folha_documento), 2)
        result["base_calculo"]["origem_base_impostos"] = "valor_base_folha_documento"
    elif base_folha_mes_anterior is not None and float(base_folha_mes_anterior) > 0:
        ...
        if document_context and document_context.get("period_mes_anterior"):
            result["base_calculo"]["period_mes_anterior"] = document_context["period_mes_anterior"]
        if document_context and document_context.get("period_mes_principal"):
            result["base_calculo"]["period_mes_principal"] = document_context["period_mes_principal"]
    else:
        folha_total = result["base_calculo"]["folha_pagamento_total"] or 0
        # Só usar folha_pagamento_total como base se for valor plausível (evitar 12, 0, etc.)
        if isinstance(folha_total, (int, float)) and folha_total >= 10_000:
            result["base_calculo"]["valor_base_folha"] = round(float(folha_total), 2)
            ...
            result["base_calculo"]["valor_base_folha"] = 0.0
            result["base_calculo"]["origem_base_impostos"] = None  # Base não identificada em faixa plausível
