...
def _fmt_brl_log(val: Optional[float]) -> str:
    """Format value for log; None/NaN -> N/A."""
    if val is None:
        return "N/A"
    try:
...
def apply_numerical_fallback(
    ...
    if total_receitas >= LIMITE_MENSAL_PLAUSIVEL:
        logger.warning(f"[FALLBACK] Total de receitas rejeitado (valor em milhões): R$ {_fmt_brl_log(total_receitas)}. Usando NÃO APURADO.")
        total_receitas = 0.0
    if total_despesas >= LIMITE_MENSAL_PLAUSIVEL:
        logger.warning(f"[FALLBACK] Total de despesas rejeitado (valor em milhões): R$ {_fmt_brl_log(total_despesas)}. Usando NÃO APURADO.")
        total_despesas = 0.0
    
    logger.info(f"[FALLBACK] Totais calculados: receitas={_fmt_brl_log(total_receitas)} ({len(receitas)} itens), despesas={_fmt_brl_log(total_despesas)} ({len(despesas)} itens)")
    ...
    if total_receitas == 0.0 and total_despesas == 0.0:
        ...
        if total_despesas_fallback:
            logger.info(f"[FALLBACK] Usando linha TOTAIS como débito: R$ {_fmt_brl_log(total_despesas_fallback)}")
            return {
                "creditos_mes": None,  # NÃO APURADO
    ...
    if total_receitas == 0.0 and total_despesas > 0.0:
        logger.info(f"[FALLBACK] Preservando despesas (R$ {_fmt_brl_log(total_despesas)}) mesmo sem receita explícita")
        return {
            "creditos_mes": None,  # NÃO APURADO
