...
def generate_dataset_financeiro(
    ...
    if (creditos_valor is None or debitos_valor is None) and df is not None and not df.empty:
        ...
        # Se ainda não encontrou valores esperados, tentar busca mais ampla em outras colunas
        if creditos_valor is None:
...
def _build_whatsapp_message(audit_result: Dict[str, Any], df: Optional[pd.DataFrame]) -> str:
    ...
    receitas = float(summary.get("total_receitas", 0))
    despesas = float(summary.get("total_despesas", 0))
    saldo = float(summary.get("saldo", receitas - despesas))
...
def format_section_3(audit_result: Dict[str, Any], df: Optional[pd.DataFrame] = None, job_id: str = "") -> Dict[str, Any]:
    ...
    # Conciliação estrutural (extração generalista): quando existir, enriquecer seção 3 e opcionalmente saldos_por_conta
    structural_extraction = doc_ctx.get("structural_extraction") if isinstance(doc_ctx, dict) else None
    if structural_extraction and isinstance(structural_extraction, dict) and not saldos_por_conta:
        # DataFrame sem coluna conta/categoria: usar contas da extração estrutural para exibição
