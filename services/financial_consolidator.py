...
from services.financial_base_validator import (
...
def calculate_financial_totals_correct(
    ...
    if extracted_totals:
        ...
        if texto_receitas is not None and texto_receitas < 1_000_000:
            # Valor mensal válido do texto - usar diretamente
            logger.info(f"[CONSOLIDATOR] Usando receitas extraídas do texto: R$ {texto_receitas:,.2f}")
            # Adicionar aos receitas extraídas se não estiver lá
            if not extracted_data.get("receitas_mensais_extraidas"):
        ...
        if texto_despesas is not None and texto_despesas < 1_000_000:
            ...
            # Adicionar aos despesas extraídas se não estiver lá
            if not extracted_data.get("despesas_mensais_extraidas"):
    ...
    from services.financial_fallback import apply_numerical_fallback
    ...
    # Usar totais do balancete apenas quando plausíveis: ambos < 1M e pelo menos um > 0 (evitar milhões)
    LIMITE_MENSAL_PLAUSIVEL = 1_000_000.0
    if soma_balancete_receita is not None or soma_balancete_despesa is not None:
    ...
    if not scale_validation.get("valid", True):
        ...
        logger.info(f"[EXTRACTION] Possível erro de escala detectado: {scale_validation.get('message')}. Receitas={total_receitas:,.2f}, Despesas={total_despesas:,.2f}. Continuando extração com incerteza marcada.")
